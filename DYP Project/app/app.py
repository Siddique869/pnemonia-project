...
with working_tab:
    ...
    st.markdown("The **ResNet-50** model's convolutional layers act as an encoder, extracting high-level features from chest X-ray images.")
    st.subheader("Classification Head")
    st.markdown("After feature extraction, a global average pooling layer and a fully connected layer are added to classify the image as **Pnemonia** or **negative**.")
    st.subheader("Training")
    st.markdown("The model is trained on a labeled dataset of chest X-ray images, with labels indicating the presence or absence of Pnemonia.")
    st.subheader("Evaluation")
    st.markdown("The model is evaluated on a separate test set to measure its accuracy, sensitivity, specificity, and other relevant metrics.")
...
with Records_tab:
    ...
    if db is not None:
        ...
        menu_option = st.sidebar.radio("Select an Option 🎯", ["Add Patient Record", "Show Patient Records"])

        if menu_option == "Add Patient Record":
        ...
        elif menu_option == "Show Patient Records":
